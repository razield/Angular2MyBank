        ...
        execute: function() {
            ChatService = (function () {
                function ChatService(http) {
                    this.http = http;
                    this._url = 'https://mybankinternal.herokuapp.com/';
                }
                ChatService.prototype.getClients = function () {
                    return this.http.get(this._url + 'clientes')
                        .map(this.extractData)
                        .catch(this.handleError);
                };
                ChatService.prototype.getMessages = function (token, email) {
                    return this.http.get(this._url + 'mensajes/' + email + '?token=' + token)
                        .map(this.extractData)
                        .catch(this.handleError);
                };
                ChatService.prototype.sendMessage = function (token, de, email, message) {
